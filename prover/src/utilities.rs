...
mod tests {
    ...
    fn test_range_check() {
        ...
        impl<const RANGE: usize> UtilitiesInstructions<jubjub::Base> for MyCircuit<RANGE> {
        ...
        impl<const RANGE: usize> Circuit<jubjub::Base> for MyCircuit<RANGE> {
            type Config = Config;
                ...
                layouter.assign_region(
                    ...
                    |mut region| {
                        ...
                        region.assign_advice(
                            ...
                            config.advice,
                            0,
                            || Value::known(jubjub::Base::from(self.0 as u64)),
                        )?;
        ...
        for i in 0..8 {
            ...
            let prover = MockProver::<jubjub::Base>::run(3, &circuit, vec![]).unwrap();
    ...
    fn test_bitrange_subset() {
        ...
        // Subset full range.
        {
            let field_elem = jubjub::Base::random(rng);
            let bitrange = 0..(jubjub::Base::NUM_BITS as usize);
        ...
        let decompose = |field_elem: jubjub::Base, ranges: &[Range<usize>]| {
            ...
            for (idx, subset) in subsets.iter().skip(1).enumerate() {
                ...
                sum += subset * jubjub::Base::from_repr(range_shift).unwrap();
            }
            assert_eq!(field_elem, sum);
        };

        decompose(jubjub::Base::random(rng), &[0..255]);
        decompose(jubjub::Base::random(rng), &[0..1, 1..255]);
        decompose(jubjub::Base::random(rng), &[0..254, 254..255]);
        decompose(jubjub::Base::random(rng), &[0..127, 127..255]);
        decompose(jubjub::Base::random(rng), &[0..128, 128..255]);
        decompose(
            jubjub::Base::random(rng),
    ...
    prop_compose! {
        fn arb_scalar()(bytes in prop::array::uniform32(0u8..)) -> jubjub::Scalar {
            ...
            jubjub::Scalar::random(&mut rand_chacha::ChaCha8Rng::from_seed(buf))
        }
    }
    ...
    proptest! {
        ...
        fn test_decompose_word(
            ...
            let decomposed = decompose_word(&scalar, jubjub::Scalar::NUM_BITS as usize, window_num_bits as usize);
            ...
            assert_eq!(scalar, jubjub::Scalar::from_repr(bytes.try_into().unwrap()).unwrap());
        }
    }
