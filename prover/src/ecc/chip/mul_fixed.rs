...
lazy_static! {
    static ref TWO_SCALAR: jubjub::Scalar = jubjub::Scalar::from(2);
    // H = 2^3 (3-bit window)
    static ref H_SCALAR: jubjub::Scalar = jubjub::Scalar::from(H as u64);
    static ref H_BASE: jubjub::Base = jubjub::Base::from(H as u64);
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct Config<FixedPoints: super::FixedPoints<jubjub::Affine>> {
    running_sum_config: RunningSumConfig<jubjub::Base, FIXED_BASE_WINDOW_SIZE>,
...
impl<FixedPoints: super::FixedPoints<jubjub::Affine>> Config<FixedPoints> {
    ...
    /// via a running sum.
    fn running_sum_coords_gate(&self, meta: &mut ConstraintSystem<jubjub::Base>) {
        meta.create_gate("Running sum coordinates check", |meta| {
            ...
            //    z_{i+1} = (z_i - a_i) / 2^3
            // => a_i = z_i - z_{i+1} * 2^3
            let word = z_cur - z_next * jubjub::Base::from(H as u64);
    ...
    fn coords_check(
        &self,
        meta: &mut VirtualCells<'_, jubjub::Base>,
        window: Expression<jubjub::Base>,
    ) -> Vec<(&'static str, Expression<jubjub::Base>)> {
        ...
        let window_pow: Vec<Expression<jubjub::Base>> = (0..H)
            .map(|pow| {
                (0..pow).fold(Expression::Constant(jubjub::Base::one()), |acc, _| {
                    acc * window.clone()
                })
            })
            .collect();
        ...
        let interpolated_x = window_pow.iter().zip(self.lagrange_coeffs.iter()).fold(
            Expression::Constant(jubjub::Base::zero()),
        ...
        let on_curve =
            y_p.square() - x_p.clone().square() * x_p - Expression::Constant(jubjub::Affine::b());
    ...
    fn assign_region_inner<F: FixedPoint<jubjub::Affine>, const NUM_WINDOWS: usize>(
        &self,
    ...
    fn assign_fixed_constants<F: FixedPoint<jubjub::Affine>, const NUM_WINDOWS: usize>(
        &self,
        region: &mut Region<'_, jubjub::Base>,
        offset: usize,
        base: &F,
        ...
        for window in 0..NUM_WINDOWS {
            ...
            region.assign_fixed(
                ...
                || {
                    ...
                    Value::known(jubjub::Base::from(z[window]))
                },
            )?;
        }
    ...
    fn process_window<F: FixedPoint<jubjub::Affine>, const NUM_WINDOWS: usize>(
        &self,
        region: &mut Region<'_, jubjub::Base>,
        offset: usize,
        w: usize,
        k_usize: Value<usize>,
        window_scalar: Value<jubjub::Scalar>,
        base: &F,
        ...
        let u_val = k_usize.map(|k| jubjub::Base::from_repr(base_u[w][k]).unwrap());
        region.assign_advice(|| "u", self.u, offset + w, || u_val)?;

        Ok(mul_b)
    }

    fn initialize_accumulator<F: FixedPoint<jubjub::Affine>, const NUM_WINDOWS: usize>(
        &self,
        region: &mut Region<'_, jubjub::Base>,
        offset: usize,
        base: &F,
    ...
    fn add_incomplete<F: FixedPoint<jubjub::Affine>, const NUM_WINDOWS: usize>(
        &self,
        region: &mut Region<'_, jubjub::Base>,
        offset: usize,
    ...
    fn process_lower_bits<F: FixedPoint<jubjub::Affine>, const NUM_WINDOWS: usize>(
        &self,
        region: &mut Region<'_, jubjub::Base>,
        offset: usize,
        w: usize,
        k: Value<jubjub::Scalar>,
        k_usize: Value<usize>,
        base: &F,
    ) -> Result<NonIdentityEccPoint, Error> {
        // `scalar = [(k_w + 2) ⋅ 8^w]
    ...
    fn process_msb<F: FixedPoint<jubjub::Affine>, const NUM_WINDOWS: usize>(
        &self,
        region: &mut Region<'_, jubjub::Base>,
        offset: usize,
        base: &F,
        ...
        let offset_acc = (0..(NUM_WINDOWS - 1)).fold(jubjub::Scalar::zero(), |acc, w| {
...
impl ScalarFixed {
    ...
    fn windows_field(&self) -> Vec<Value<jubjub::Scalar>> {
        let running_sum_to_windows = |zs: Vec<AssignedCell<jubjub::Base, jubjub::Base>>| {
            (0..(zs.len() - 1))
                .map(|idx| {
                    let z_cur = zs[idx].value();
                    let z_next = zs[idx + 1].value();
                    let word = z_cur - z_next * Value::known(*H_BASE);
                    // This assumes that the endianness of the encodings of jubjub::Base
                    // and jubjub::Scalar are the same. They happen to be, but we need to
                    // be careful if this is generalised.
                    word.map(|word| jubjub::Scalar::from_repr(word.to_repr()).unwrap())
                })
                .collect::<Vec<_>>()
        };
        match self {
            ...
            Self::FullWidth(scalar) => scalar
                ...
                .map(|bits| {
                    ...
                    // be careful if this is generalised.
                    bits.value()
                        .map(|value| jubjub::Scalar::from_repr(value.to_repr()).unwrap())
                })
                .collect::<Vec<_>>(),
        }
    }
