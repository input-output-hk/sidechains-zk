...
impl<Fixed: FixedPoints<jubjub::Affine>> Config<Fixed> {
    ...
    pub fn assign(
        ...
        base: &<Fixed as FixedPoints<jubjub::Affine>>::ShortScalar,
    ) -> Result<(EccPoint, EccScalarFixedShort), Error>
    where
        <Fixed as FixedPoints<jubjub::Affine>>::ShortScalar:
            super::super::FixedPoint<jubjub::Affine>,
    {
            ...
            scalar
                ...
                .assert_if_known(|((magnitude, sign), result)| {
                    let magnitude_is_valid =
                        magnitude <= &&jubjub::Base::from(0xFFFF_FFFF_FFFF_FFFFu64);
                    let sign_is_valid = sign.square() == jubjub::Base::one();
                    ...
                    !(magnitude_is_valid && sign_is_valid) || {
                        let scalar = {
                            ...
                            // for Pallas).
                            let magnitude = jubjub::Scalar::from_repr(magnitude.to_repr()).unwrap();

                            let sign = if sign == &&jubjub::Base::one() {
                                jubjub::Scalar::ONE
                            } else {
                                -jubjub::Scalar::ONE
                            };
...
pub mod tests {
    ...
    pub(crate) fn test_mul_fixed_short(
        ...
        fn constrain_equal_non_id(
            chip: EccChip<TestFixedBases>,
            mut layouter: impl Layouter<jubjub::Base>,
            base_val: jubjub::Affine,
            scalar_val: jubjub::Scalar,
            result: Point<jubjub::Affine, EccChip<TestFixedBases>>,
        ) -> Result<(), Error> {
        ...
        let magnitude_signs = [
            ("random [a]B", jubjub::Base::from(rand::random::<u64>()), {
                let mut random_sign = jubjub::Base::one();
                if rand::random::<bool>() {
            ...
            (
                "[2^64 - 1]B",
                jubjub::Base::from(0xFFFF_FFFF_FFFF_FFFFu64),
                jubjub::Base::one(),
            ),
            (
                "-[2^64 - 1]B",
            ...
            (
                "mul_with_double",
                jubjub::Base::from(0xB6DB_6DB6_DB6D_B6DCu64),
                jubjub::Base::one(),
            ),
            (
        ...
        let zero_magnitude_signs = [
            ("mul by +zero", jubjub::Base::zero(), jubjub::Base::one()),
            ("mul by -zero", jubjub::Base::zero(), -jubjub::Base::one()),
        ];
    ...
    fn invalid_magnitude_sign() {
        ...
        #[derive(Default)]
        struct MyCircuit {
            magnitude: Value<jubjub::Base>,
            sign: Value<jubjub::Base>,
            // For test checking
            magnitude_error: Value<jubjub::Base>,
        }

        impl UtilitiesInstructions<jubjub::Base> for MyCircuit {
            type Var = AssignedCell<jubjub::Base, jubjub::Base>;
        }

        impl Circuit<jubjub::Base> for MyCircuit {
        ...
        fn format_value(v: jubjub::Base) -> String {
            use ff::Field;
            if v.is_zero_vartime() {
                "0".into()
            } else if v == jubjub::Base::one() {
                "1".into()
            } else if v == -jubjub::Base::one() {
                "-1".into()
            } else {
        ...
        // Magnitude larger than 64 bits should fail
        {
            let circuits = [
                // 2^64
                MyCircuit {
                    magnitude: Value::known(jubjub::Base::from_u128(1 << 64)),
                    sign: Value::known(jubjub::Base::one()),
                    magnitude_error: Value::known(jubjub::Base::from(1 << 1)),
                },
                // -2^64
                MyCircuit {
                    magnitude: Value::known(jubjub::Base::from_u128(1 << 64)),
                ...
                // 2^66
                MyCircuit {
                    magnitude: Value::known(jubjub::Base::from_u128(1 << 66)),
                    sign: Value::known(jubjub::Base::one()),
                    magnitude_error: Value::known(jubjub::Base::from(1 << 3)),
                },
                // -2^66
                MyCircuit {
                    magnitude: Value::known(jubjub::Base::from_u128(1 << 66)),
                ...
                // 2^254
                MyCircuit {
                    magnitude: Value::known(jubjub::Base::from_u128(1 << 127).square()),
                    sign: Value::known(jubjub::Base::one()),
                    magnitude_error: Value::known(
                        jubjub::Base::from_u128(1 << 95).square() * jubjub::Base::from(2),
                    ),
                },
                // -2^254
                MyCircuit {
            ...
            for circuit in circuits.iter() {
                let prover = MockProver::<jubjub::Base>::run(11, circuit, vec![]).unwrap();
        ...
        // Sign that is not +/- 1 should fail
        {
            ...
            let circuit = MyCircuit {
                magnitude: Value::known(jubjub::Base::from(magnitude_u64)),
                sign: Value::known(jubjub::Base::zero()),
                magnitude_error: Value::unknown(),
            };

            let negation_check_y = {
                *(Short.generator() * jubjub::Scalar::from(magnitude_u64))
                    .to_affine()
                    .coordinates()
                    .unwrap()
                    .y()
