...
pub struct Config<Fixed: FixedPoints<jubjub::Affine>> {
    ...
    lookup_config: LookupRangeCheckConfig<jubjub::Base, { sinsemilla::K }>,
...
impl<Fixed: FixedPoints<jubjub::Affine>> Config<Fixed> {
    ...
    fn create_gate(&self, meta: &mut ConstraintSystem<jubjub::Base>) {
        ...
        meta.create_gate("Canonicity checks", |meta| {
            ...
            let alpha_0 = {
                let two_pow_252 = jubjub::Base::from_u128(1 << 126).square();
            ...
            let decomposition_checks = {
                ...
                let z_84_alpha_check = z_84_alpha.clone()
                    - (alpha_1.clone() + alpha_2.clone() * jubjub::Base::from(1 << 2));
            ...
            let alpha_0_prime_check = {
                let two_pow_130 = Expression::Constant(jubjub::Base::from_u128(1 << 65).square());
                let t_p = Expression::Constant(jubjub::Base::from_u128(T_P));
            ...
            let canon_checks = {
                ...
                let alpha_0_hi_120 = {
                    let two_pow_120 =
                        Expression::Constant(jubjub::Base::from_u128(1 << 60).square());
    ...
    pub fn assign(
        ...
        scalar: AssignedCell<jubjub::Base, jubjub::Base>,
        base: &<Fixed as FixedPoints<jubjub::Affine>>::Base,
    ) -> Result<EccPoint, Error>
    where
        <Fixed as FixedPoints<jubjub::Affine>>::Base: super::super::FixedPoint<jubjub::Affine>,
    {
        ...
        // Check that the correct multiple is obtained.
        {
            ...
            let scalar = &scalar
                .base_field_elem()
                .value()
                .map(|scalar| jubjub::Scalar::from_repr(scalar.to_repr()).unwrap());
        ...
        let (alpha_0_prime, z_13_alpha_0_prime) = {
            ...
            let alpha_0_prime = alpha_0.map(|alpha_0| {
                let two_pow_130 = jubjub::Base::from_u128(1 << 65).square();
                let t_p = jubjub::Base::from_u128(T_P);
...
pub mod tests {
    ...
    fn test_single_base(
        chip: EccChip<TestFixedBases>,
        mut layouter: impl Layouter<jubjub::Base>,
        base: FixedPointBaseField<jubjub::Affine, EccChip<TestFixedBases>>,
        ...
        // [a]B
        {
            let scalar_fixed = jubjub::Base::random(rng);
            let result = {
        ...
        {
            let h = jubjub::Base::from(H as u64);
            let scalar_fixed = "1333333333333333333333333333333333333333333333333333333333333333333333333333333333334"
                        .chars()
                        .fold(jubjub::Base::zero(), |acc, c| {
                            acc * &h + &jubjub::Base::from(c.to_digit(8).unwrap() as u64)
                        });
            let result = {
                let scalar_fixed = chip.load_private(
        ...
        // on the last step.
        {
            let scalar_fixed = jubjub::Base::zero();
            let result = {
                let scalar_fixed = chip.load_private(
        ...
        {
            let scalar_fixed = -jubjub::Base::one();
            let result = {
                let scalar_fixed = chip.load_private(
