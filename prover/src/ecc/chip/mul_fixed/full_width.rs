...
impl<Fixed: FixedPoints<jubjub::Affine>> Config<Fixed> {
    ...
    fn witness(
        ...
        offset: usize,
        scalar: Value<jubjub::Scalar>,
    ) -> Result<EccScalarFixed, Error> {
        let windows = self.decompose_scalar_fixed::<{ jubjub::Scalar::NUM_BITS as usize }>(
    ...
    fn decompose_scalar_fixed<const SCALAR_NUM_BITS: usize>(
        ...
        offset: usize,
        region: &mut Region<'_, jubjub::Base>,
    ) -> Result<ArrayVec<AssignedCell<jubjub::Base, jubjub::Base>, NUM_WINDOWS>, Error> {
        ...
        let scalar_windows: Value<Vec<u8>> = scalar.map(|scalar| {
            decompose_word::<jubjub::Scalar>(&scalar, SCALAR_NUM_BITS, FIXED_BASE_WINDOW_SIZE)
        });

        // Transpose `Value<Vec<u8>>` into `Vec<Value<jubjub::Base>>`.
        let scalar_windows = scalar_windows
            .map(|windows| {
                windows
                    .into_iter()
                    .map(|window| jubjub::Base::from(window as u64))
            })
            .transpose_vec(NUM_WINDOWS);

        // Store the scalar decomposition
        let mut windows: ArrayVec<AssignedCell<jubjub::Base, jubjub::Base>, NUM_WINDOWS> =
            ArrayVec::new();
    ...
    pub fn assign(
        ...
        base: &<Fixed as FixedPoints<jubjub::Affine>>::FullScalar,
    ) -> Result<(EccPoint, EccScalarFixed), Error>
    where
        <Fixed as FixedPoints<jubjub::Affine>>::FullScalar:
...
pub mod tests {
    ...
    fn test_single_base(
        chip: EccChip<TestFixedBases>,
        mut layouter: impl Layouter<jubjub::Base>,
        base: FixedPoint<jubjub::Affine, EccChip<TestFixedBases>>,
        ...
        // [a]B
        {
            let scalar_fixed = jubjub::Scalar::random(OsRng);
        ...
        // 5333333333333333333333333333333333333333332711161673731021062440252244051273333333333 in octal.)
        {
            ...
            let h = jubjub::Scalar::from(H as u64);
            let scalar_fixed = LAST_DOUBLING
                .chars()
                .fold(jubjub::Scalar::zero(), |acc, c| {
                    acc * &h + &jubjub::Scalar::from(c.to_digit(8).unwrap() as u64)
                });
            let by = ScalarFixed::new(
                chip.clone(),
        ...
        // on the last step.
        {
            let scalar_fixed = jubjub::Scalar::zero();
        ...
        {
            let scalar_fixed = -jubjub::Scalar::ONE;
