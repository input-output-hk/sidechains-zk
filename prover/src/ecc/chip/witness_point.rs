...
type Coordinates = (
    AssignedCell<Assigned<jubjub::Base>, jubjub::Base>,
    AssignedCell<Assigned<jubjub::Base>, jubjub::Base>,
);
...
impl Config {
    ...
    fn create_gate(&self, meta: &mut ConstraintSystem<jubjub::Base>) {
        let curve_eqn = |meta: &mut VirtualCells<jubjub::Base>| {
            let x_square = meta.query_advice(self.x, Rotation::cur()).square();
            let y_square = meta.query_advice(self.y, Rotation::cur()).square();
        ...
        meta.create_gate("witness non-identity point", |meta| {
            // Check that the point being witnessed is a valid curve point -x^2 + y^2 = 1 + d * x^2 * y^2
    ...
    fn assign_xy(
        &self,
        value: Value<(Assigned<jubjub::Base>, Assigned<jubjub::Base>)>,
        offset: usize,
    ...
    pub(super) fn point_non_id(
        ...
        value.error_if_known_and(|value| value == &jubjub::AffinePoint::identity())?;
...
pub mod tests {
    ...
    pub fn test_witness_non_id<
        EccChip: EccInstructions<jubjub::AffinePoint> + Clone + Eq + std::fmt::Debug,
    >(
        chip: EccChip,
        ...
        NonIdentityPoint::new(
            ...
            Value::known(jubjub::AffinePoint::identity()),
        )
