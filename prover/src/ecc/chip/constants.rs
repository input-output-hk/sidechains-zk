...
pub fn find_zs_and_us<C: CurveAffine>(
    ...
    let find_z_and_us = |window_points: &[C]| {
        ...
        let ys: Vec<_> = window_points
            ...
            .collect();
        println!("inside, size vec: {:?}", ys.len());
        (0..(1000 * (1 << (2 * H)))).find_map(|z| {
            let mut vec_test: Vec<bool> = Vec::new();
            let a = ys.iter()
                .map(|&y| {
                    if (-y + C::Base::from(z)).sqrt().is_none().into() {
                        vec_test.push((y + C::Base::from(z)).sqrt().is_some().into());
            ...
            println!("vec: {:?}", vec_test);
            if a.is_some() {
                println!("Seems to be some");
            }
            a.map(|us| {
                    println!("Inside the final fn");
                    (z, us.into_inner().unwrap())
            })
        })
    };
...
mod tests {
    ...
    prop_compose! {
        pub fn arb_point()(bytes in prop::array::uniform32(0u8..)) -> jubjub::AffinePoint {
            // Instead of rejecting out-of-range bytes, let's reduce them.
            let mut buf = [0; 32];
            buf[..32].copy_from_slice(&bytes);
            let scalar = jubjub::Scalar::random(&mut &mut rand_chacha::ChaCha8Rng::from_seed(buf));
            (jubjub::AffinePoint::generator() * scalar).to_affine()
        }
    }
    ...
    #[test]
    fn zs_and_us() {
        let base = jubjub::ExtendedPoint::random(rand::rngs::OsRng).to_affine();
        println!("Let's try this");
        let (z, u): (Vec<u64>, Vec<[jubjub::Base; H]>) =
            find_zs_and_us(base, NUM_WINDOWS)
                .unwrap()
                .into_iter()
                .unzip();
        println!("Let's try two");
        let window_table = compute_window_table(base, NUM_WINDOWS);

        for ((u, z), window_points) in u.iter().zip(z.iter()).zip(window_table) {
            for (u, point) in u.iter().zip(window_points.iter()) {
                let y = *point.coordinates().unwrap().y();
                assert_eq!(jubjub::Base::from(*z) + y, u * u); // allow either square root
                assert!(bool::from((jubjub::Base::from(*z) - y).sqrt().is_none()));
            }
        }
    }
}
