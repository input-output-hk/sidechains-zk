...
pub struct Config {
    // Selector used to constrain the cells used in addition.
    q_mul_decompose_var: Selector,
    // Advice column used to decompose scalar in addition.
...
impl Config {
    ...
    fn create_gate(&self, meta: &mut ConstraintSystem<jubjub::Base>) {
        ...
        meta.create_gate(
            ...
            |meta| {
                ...
                // k_{i} = z_{i} - 2⋅z_{i+1}
                let k = z_next - Expression::Constant(jubjub::Base::from(2)) * z_prev;
                // (k_i) ⋅ (1 - k_i) = 0
    ...
    pub(super) fn assign_region(
        ...
        y_a: Y<jubjub::Base>,
        z: Z<jubjub::Base>,
    ) -> Result<(EccPoint, Vec<Z<jubjub::Base>>), Error> {
        ...
        let mut zs: Vec<Z<jubjub::Base>> = Vec::with_capacity(bits.len());
        ...
        for (iter, k) in bits.iter().enumerate() {
            ...
            z = {
                ...
                let z_val = z.value() * Value::known(jubjub::Base::from(2))
                    + k.map(|k| jubjub::Base::from(k as u64));
                let z_cell =
