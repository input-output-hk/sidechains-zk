...
pub struct Config {
    ...
    // 10-bit lookup table
    lookup_config: LookupRangeCheckConfig<jubjub::Base, { super::K }>,
    // Advice columns
...
impl Config {
    ...
    fn create_gate(&self, meta: &mut ConstraintSystem<jubjub::Base>) {
        ...
        meta.create_gate("overflow checks", |meta| {
            ...
            let two_pow_124 = Expression::Constant(jubjub::Base::from_u128(1 << 124));
            let two_pow_130 =
                two_pow_124.clone() * Expression::Constant(jubjub::Base::from_u128(1 << 6));
            ...
            let t_q = Expression::Constant(jubjub::Base::from_u128(T_Q));
    ...
    pub(super) fn overflow_check(
        ...
        zs: &[Z<jubjub::Base>], // [z_0, z_1, ..., z_{254}, z_{255}]
    ) -> Result<(), Error> {
        ...
        let s = {
            ...
            let s_val = alpha
                .value()
                .zip(k_254.value())
                .map(|(alpha, k_254)| alpha + k_254 * jubjub::Base::from_u128(1 << 65).square());
    ...
    fn s_minus_lo_130(
        &self,
        mut layouter: impl Layouter<jubjub::Base>,
        s: AssignedCell<jubjub::Base, jubjub::Base>,
    ) -> Result<AssignedCell<jubjub::Base, jubjub::Base>, Error> {
        // Number of k-bit words we can use in the lookup decomposition.
        let num_words = 130 / super::K;
        assert!(num_words * super::K == 130);
