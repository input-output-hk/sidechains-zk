...
impl<F: PrimeFieldBits, const WINDOW_NUM_BITS: usize> RunningSumConfig<F, WINDOW_NUM_BITS> {
    ...
    fn decompose(
        ...
        // `z` column at `offset`.
        let two_pow_k_inv = Value::known(F::from(1 << WINDOW_NUM_BITS as u64).invert().unwrap()); // todo: do we not need to convert this?
...
mod tests {
    ...
    const L_BASE: usize = jubjub::Scalar::NUM_BITS as usize;
    ...
    fn test_running_sum() {
        ...
        {
            let alpha_bytes = jubjub::Scalar::random(OsRng).to_bytes();
            let alpha = jubjub::Base::from_bytes(&alpha_bytes).unwrap();

            // Strict full decomposition should pass.
            let circuit: MyCircuit<jubjub::Base, L_BASE, FIXED_BASE_WINDOW_SIZE, { NUM_WINDOWS }> =
                MyCircuit {
                    alpha: Value::known(alpha),
                    strict: true,
                };
            let prover = MockProver::<jubjub::Base>::run(8, &circuit, vec![]).unwrap();
            assert_eq!(prover.verify(), Ok(()));
        }

        // Random 64-bit word
        {
            let alpha = jubjub::Base::from(rand::random::<u64>());
        ...
        // 2^66
        {
            let alpha = jubjub::Base::from_u128(1 << 66);
